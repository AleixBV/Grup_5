			...
			switch (e->mov_type)
			{
				...
				case eCurv:
				{
					...
					case _WORM:
					{
						...
						else if (90 > e->position.y && e->position.y >= 100)
						{
							unsigned int frame_num = (e->position.y - 80) / 10 ;
...
void ModuleEnemy::OnCollision(Collider* c1, Collider* c2)
{
	...
	while (tmp != NULL)
	{
		...
		if (e->on_screen)
		{
			...
			{
				if (c1->type == COLLIDER_PLAYER_SHOT || c2->type == COLLIDER_PLAYER_SHOT || c1->type == COLLIDER_PLAYER_SHOT2 || c2->type == COLLIDER_PLAYER_SHOT2 || c1->type == COLLIDER_PLAYER_SHOT_CHARGED || c2->type == COLLIDER_PLAYER_SHOT_CHARGED || c1->type == COLLIDER_PLAYER_SHOT_POWERUP || c2->type == COLLIDER_PLAYER_SHOT_POWERUP)
				{
					e->alive = false;
